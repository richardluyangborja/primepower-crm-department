              ...
              <TableRow
                ...
                onClick={() =>
                  navigate({
                    ...
                    params: { companyId: companyLead.id.toString() },
                  })
                }
              >
                <TableCell className="font-medium">
                  <div className="flex flex-col">
                    <span>{companyLead.company}</span>
                    <span className="text-xs font-normal text-muted-foreground">
                      {companyLead.industry}
                    </span>
                  </div>
                </TableCell>
                <TableCell>{companyLead.source}</TableCell>
                <TableCell className="flex flex-col">
                  <span>{companyLead.contacts.at(0)?.name}</span>
                  <span className="text-xs text-muted-foreground">
                    {companyLead.contacts.at(0)?.position}
                  </span>
                </TableCell>
                <TableCell>
                  <Badge variant="secondary">{companyLead.lead.status}</Badge>
                </TableCell>
                <TableCell>{companyLead.salesRepresentative}</TableCell>
                <TableCell>
                  {companyLead.lead.lastActivity.toDateString()}
                </TableCell>
                <TableCell>
                  <DropdownMenu>
